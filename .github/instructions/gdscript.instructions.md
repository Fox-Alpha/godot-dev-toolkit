...
- Use **gdscript-formatter** (`skills/gdscript-formatter.md`) to format and lint GDScript files
- CLI: `gdscript-formatter --safe path/to/file.gd` — format in place
- CLI: `gdscript-formatter lint --pretty path/to/file.gd` — check style issues
- VSCode: install `DoHe.godot-format` extension for format-on-save
- Before committing: run `skills/pre-commit-check.md` workflow
